...
CONFIG = {
  ...
  'project' => File.join(SOURCE, "projects/_posts"),
  'posts' => File.join(SOURCE, "_posts"),
  'post_ext' => "md",
}

# Usage: rake person name="A Title" [date="2012-02-09"]
desc "Begin a new person in #{CONFIG['person']}"
task :person do
  abort("rake aborted: '#{CONFIG['person']}' directory not found.") unless FileTest.directory?(CONFIG['person'])
  title = ENV["name"] || "new-person"
  ...
  filename = File.join(CONFIG['person'], "#{date}-#{slug}.#{CONFIG['post_ext']}")
  if File.exist?(filename)
    abort("rake aborted!") if ask("#{filename} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
  end
  
  puts "Creating new person: #{filename}"
  open(filename, 'w') do |person|
    person.puts "---"
    person.puts "layout: person"
    person.puts "category: team"
    person.puts "name: \"#{title.gsub(/-/,' ')}\""
    person.puts "designation: PhD Candidate"
    person.puts "thumbnail:"
    person.puts "homepage:"
    person.puts "googleplus:"
    person.puts "twitter:"
    person.puts "facebook:"
    person.puts "linkedin:"
    person.puts "email:"
    person.puts "github:"
    person.puts "pinboard:"
    person.puts "slideshare:"
    person.puts "vimeo:"
    person.puts "---"
  end
end # task :person
...
desc "Begin a new project in #{CONFIG['project']}"
task :project do
  abort("rake aborted: '#{CONFIG['project']}' directory not found.") unless FileTest.directory?(CONFIG['project'])
  ...
  filename = File.join(CONFIG['project'], "#{date}-#{slug}.#{CONFIG['post_ext']}")
  # filename = File.join(CONFIG['project'], "#{slug}.#{CONFIG['post_ext']}")
  if File.exist?(filename)
    abort("rake aborted!") if ask("#{filename} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
  end
  ...
  open(filename, 'w') do |project|
    ...
    project.puts "subtitle: \"#{title.gsub(/-/,' ')}\""
    project.puts 'thumbnail: '
    project.puts 'excerpt: "" '
    project.puts "tags: []"
    project.puts "---"
  end
end # task project:
